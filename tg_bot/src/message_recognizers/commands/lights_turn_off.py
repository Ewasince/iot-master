...
class CommandLightsTurnOff(ICommandPerformer):
    ...
    async def perform_command(self, command_context: str) -> str | None:
        ...
        res = await light.turn_off()

        if res:
            return f'Лампочка "{light.name}" успешно выключена'
        else:
            return f'Не удалось выключить лампочку "{light.name}'
